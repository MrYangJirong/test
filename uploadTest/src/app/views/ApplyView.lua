...
function ApplyView:loadData(desk)
	local apply = desk.info.apply
	local applyEx = desk.info.applyEx
	...
	if applyEx then
		if self.bCountDownBegin == false then
			...
			self.nodeCountDown:setVisible(true)
	...
	self.MainPanel.list:setContentSize(cc.size(self.listSize.width*playerNum,self.listSize.height))
	local isprocessed = false
	local idx = 0
    
	for uid, status in pairs(apply.result) do
		self.MainPanel.list:pushBackDefaultItem()
		local _, player = desk:getPlayerPos(uid)
		local isHorse = false
		
		if not player then
			if desk:isHorsePlayer(uid) then
				player = desk.info.horse
				isHorse = true
			end
		end
		...
		if player then
			...
			elseif status == 1 then
				...
				path = 'views/jiesan/yifandui.png'
				if meUid == uid then
					isprocessed = true
				end
			end
